...
export class TradeRepository extends Repository<Trade> {
	...
	async getAllTradeIds(): Promise<string[]> {
		try {
			const trades = await this.createQueryBuilder('trade')
				.select('trade.tradeId') // tradeId 컬럼만 선택
				.getMany();

			return trades.map((trade) => trade.tradeId.toString());
		} catch (error) {
			this.logger.error(`Failed to fetch all trade IDs`, {
				error: error.stack,
			});
			throw new Error('Failed to fetch all trade IDs');
		}
	}
}
